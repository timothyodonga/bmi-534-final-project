...
config = {
    "fine_tune_train": f"C:/Users/timot/OneDrive/Desktop/EMORY/Spring 2024/BMI-534/project-code/code/bmi-534-final-project/saved_models/harth_train_fold_{fold}.pt",
    "fine_tune_test": f"C:/Users/timot/OneDrive/Desktop/EMORY/Spring 2024/BMI-534/project-code/code/bmi-534-final-project/saved_models/harth_test_fold_{fold}.pt",
...
for epoch in range(1, config["num_epochs"] + 1):
    ...
    if config["model_type"] != "cnn" and config["model_type"] != "cnn_small":
        train_loss, emb_finetune, label_finetune, F1 = model_finetune(
            model=TFC_model,
    ...
    if valid_loss < gvalid_loss or valid_f1 > max_f1:
        ...
        if config["model_type"] == "cnn_small":
            torch.save(
                ...
                + config["arch"]
                + f"_cnn_small_tfc_fold_{fold}_model.pt",
            )
            torch.save(
                classifier.state_dict(),
                "experiments_logs/finetunemodel/"
                + config["arch"]
                + f"_cnn_small_fold_{fold}_classifier.pt",
            )
        elif config["model_type"] == "cnn":
            torch.save(
                TFC_model.state_dict(),
                "experiments_logs/finetunemodel/"
                + config["arch"]
                + f"_cnn_tfc_fold_{fold}_model.pt",
            )
            torch.save(
                classifier.state_dict(),
                "experiments_logs/finetunemodel/"
                + config["arch"]
                + f"_cnn_fold_{fold}_classifier.pt",
            )
...
df_performance = pd.DataFrame(
    {
        ...
        "F1 (macro)": [f1],
        "AUC": [roc_value],
        "model": [f"cnn_small_fold_{fold}_classifier"],
    }
)

# if fold == "0":
#     df_performance.to_csv("tfc_finetuned_small_cnn_.csv", index=False, mode="a")
# else:
#     df_performance.to_csv(
#         "tfc_finetuned_small_cnn_.csv", index=False, mode="a", header=None
#     )
