...
def generate_freq(dataset, config):
    ...
    X_train, y_train = torch.stack(list(X_train), dim=0), torch.stack(
        list(y_train), dim=0
    )
...
class Load_Dataset(Dataset):
    ...
    def __init__(
        self, dataset, config, training_mode, target_dataset_size=64, subset=False
    ):
        ...
        self.x_data_f = fft.fft(
            self.x_data
        ).abs()  # /(window_length) # rfft for real value inputs.
...
class Load_DatasetTwo(Dataset):
    ...
    def __init__(
        ...
        config,
        training_mode,
        target_dataset_size=64,
        subset=False,
        valid=False,
        train_valid_split=0.8,
    ):
        super(Load_DatasetTwo, self).__init__()
        self.training_mode = training_mode
        X_train = dataset["samples"]
        y_train = dataset["labels"]
        # shuffle
        ...
        # Added a line of code to get the validation data
        if valid == True:
            X_train = X_train[int(train_valid_split * len(X_train)) :]
            y_train = y_train[int(train_valid_split * len(y_train)) :]
        else:
            X_train = X_train[: int(train_valid_split * len(X_train))]
            y_train = y_train[: int(train_valid_split * len(y_train))]
