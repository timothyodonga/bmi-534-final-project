...
def model_test(
    ...
    with torch.no_grad():
        ...
        for data, labels, _, data_f, _ in test_dl:
            ...
            print("****Printing the shape of the concatenated features")
            print(fea_concat.shape)
            print("Printing the shape of the data")
            print(data.shape)

            print("Printing the shape of z_t and z_f")
            print(z_t.shape)
            print(z_f.shape)

            print("====================================================")
            # predictions_test = classifier(fea_concat)
            print(fea_concat__.shape)
            predictions_test = classifier(fea_concat__)
            fea_concat_flat = fea_concat.reshape(fea_concat.shape[0], -1)
            print("Printing the shape of fea_concat_flat")
            print(fea_concat_flat.shape)
